...
const LLM_MODEL = "qwen3-fast:latest";
                ...
                <div style={{
                  ...
                  {m.role === "user" ? m.content : (
                      <Markdown components={{
                        ...
                        strong: ({children}) => <strong style={{color:"var(--orange)",fontWeight:700}}>{children}</strong>,
                        em: ({children}) => <em style={{color:"var(--text2)"}}>{children}</em>,
                        code: ({children}) => <code style={{background:"var(--surface3)",padding:"1px 5px",borderRadius:4,fontSize:12,fontFamily:"'JetBrains Mono',monospace"}}>{children}</code>,
                        ul: ({children}) => <ul style={{margin:"4px 0",paddingLeft:18}}>{children}</ul>,
                        ol: ({children}) => <ol style={{margin:"4px 0",paddingLeft:18}}>{children}</ol>,
                        li: ({children}) => <li style={{margin:"2px 0"}}>{children}</li>,
                        h1: ({children}) => <div style={{fontSize:16,fontWeight:700,margin:"6px 0 4px"}}>{children}</div>,
                        h2: ({children}) => <div style={{fontSize:15,fontWeight:700,margin:"6px 0 4px"}}>{children}</div>,
                        h3: ({children}) => <div style={{fontSize:14,fontWeight:700,margin:"4px 0 2px"}}>{children}</div>,
                        a: ({href,children}) => <a href={href} target="_blank" rel="noopener noreferrer" style={{color:"var(--orange)",textDecoration:"underline"}}>{children}</a>,
                        pre: ({children}) => <pre style={{background:"var(--surface3)",padding:10,borderRadius:8,overflowX:"auto",margin:"6px 0",fontSize:12}}>{children}</pre>,
                      }}>{m.content}</Markdown>
                  )}
                </div>
              </div>
